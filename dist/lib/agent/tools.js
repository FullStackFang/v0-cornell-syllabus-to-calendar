...
export const toolSchemas = {
    create_calendar_events: {
        description: "Create events on the user's Google Calendar. Use this when the user wants to add course sessions, assignments, or exams to their calendar.",
        parameters: z.object({
    ...
    search_emails: {
        description: "Search the user's Gmail for emails. Use Gmail search syntax: 'from:email@example.com', 'subject:keyword', 'after:2024/01/01', etc. Use this to find course-related emails from professors or about assignments.",
        parameters: z.object({
    ...
    summarize_email_thread: {
        description: "Get an email thread and summarize its contents. Use this when the user wants to understand a conversation thread about a course topic.",
        parameters: z.object({
            threadId: z.string().describe("The Gmail thread ID to fetch and summarize"),
        }),
    },
    parse_syllabus: {
        description: "Extract structured course data from syllabus text. Use this when the user uploads a syllabus or provides syllabus content to parse.",
        parameters: z.object({
            syllabusText: z.string().describe("The raw text content from a syllabus PDF"),
        }),
    },
};
export async function executeCreateCalendarEvents(accessToken, params) {
...
export async function executeSearchEmails(accessToken, params) {
...
export async function executeSummarizeEmailThread(accessToken, params) {
...
export async function executeParseSyllabus(params) {
