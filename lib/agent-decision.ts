...
import { ClaudeModel, MODEL_IDS, MODEL_INFO, CourseSettings } from "./course-config"
...
const DEFAULT_MODEL: ClaudeModel = "haiku"
...
function getNextModel(currentModel: ClaudeModel): ClaudeModel | null {
...
export async function analyzeQuestion(
  ...
  if (
    ...
    if (smarterModel) {
      console.log(
        `Low confidence (${(decision.confidence * 100).toFixed(0)}%), retrying with ${smarterModel}`
      )
      const smarterDecision = await callClaude(
        email,
        kbContext,
        matchedFaqs,
        smarterModel,
...
async function callClaude(
  email: EmailData,
  kbContext: string,
  matchedFaqs: Array<{ question: string; answer: string; similarity: number; id: string }>,
  model: ClaudeModel,
...
export function formatProfessorNotification(
  email: EmailData,
  decision: Decision,
  approvalLinks: {
    approve: string
 ...
 */
export function createOptionsFromSettings(
 ...
 */
export function estimateCost(model: ClaudeModel): { input: number; output: number; total: number } {
