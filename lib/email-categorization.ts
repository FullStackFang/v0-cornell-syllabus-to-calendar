 ...
 */
export function extractSignificantKeywords(courseName: string): string[] {
  return courseName
    .split(/\s+/)
    .filter(word =>
      word.length > 3 &&
 ...
 */
export function buildCourseEmailQuery(course: Course): string {
  const parts: string[] = []
...
// Categorization rules with regex patterns
...
export function groupEmails(
  ...
  const categories: EmailCategory[] = ["assignments", "announcements", "schedule_changes", "general"]
